...
void IntegrationManager::capture() {
  ...
  std::string MOSSOldPath = "MOSS_OLD_PATH=" + OldPath;
  std::string Path = "PATH=" + ExtendedPath;
  std::string MOSSOutputDir = "MOSS_OUTPUT_DIR=" + OptionManager::OutputDir;
  
  std::vector<llvm::StringRef> Env = getAllEnv();
  Env.push_back(MOSSOldPath);
  Env.push_back(Path);
  Env.push_back(MOSSOutputDir);
  ...
  if (Program) {
    ...
    //construct su command
    llvm::ErrorOr<std::string> su = llvm::sys::findProgramByName(
      "su", llvm::ArrayRef<llvm::StringRef>(PathVecRef));
    if(su){
        std::string Command = OptionManager::BuildCmd[0];
           std::vector<llvm::StringRef> SuArgs = {"su", "postgres", "-m", "-c", Command};
        ...
        llvm::errs() << "Error: Cannot find su command"
...
void IntegrationManager::buildCompilationDB() {
  ...
  llvm::errs() << CaptureFilePath <<"\n";
  if (!CaptureFile.is_open()) {
    //llvm::errs() << "Error: Unable to open capture.txt\n";
    // exit(1);
    for(std::string p : OptionManager::ProgramList){
      OptionManager::InputFiles.push_back(p);
      llvm::errs()<<p<<"\n";
    }
  }
  ...
  for (auto &E : CompilationDB)
  {
    if (OptionManager::ProgramListFile.compare("none") != 0)
    {
      if (std::count(OptionManager::ProgramList.begin(), OptionManager::ProgramList.end(), E.first) != 0)
      {
