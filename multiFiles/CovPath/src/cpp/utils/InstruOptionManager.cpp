...
const std::string usage_simple("Usage: domgad [OPTIONS]... ORACLE PROGRAM");
const std::string error_message("Try 'domgad --help' for more information.");
...
void InstruOptionManager::showUsage() {
  llvm::errs()
      ...
      << "  --build                Integrate DomGad with build system\n"
...
std::string InstruOptionManager::OutputDir = "domgad-out";
...
void InstruOptionManager::handleOptions(int argc, char *argv[]) {
  ...
  if (optind + 2 > argc && !InstruOptionManager::Stat) {
    llvm::errs() << "domgad: You must specify the oracle and input.\n";
    llvm::errs() << usage_simple << "\n";
    llvm::errs() << error_message << "\n";
    exit(1);
  } else if (optind + 1 > argc && InstruOptionManager::Stat) {
    llvm::errs() << "domgad: You must specify the input.\n";
    exit(1);
  }
  ...
  if (Build) {
    // integrate DomGad with build system
  ...
  // When DomGad only computes statistics, it does not need to check the oracle
