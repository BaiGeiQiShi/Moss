...
bool StmtInstrumentation::HandleTopLevelDecl(DeclGroupRef DR)
{
  ...
  {
    if (SourceManager::IsInHeader(SM, *b))
      continue;
...
bool StmtCollectionVisitor::VisitFunctionBodyCompoundStmtForPrint(const CompoundStmt *cs)
{
  ...
  {
    // std::cout << "Processing bi: " << *bi << std::endl;
...
bool StmtCollectionVisitor::VisitStmtForPrint(const Stmt *stmt)
{
  ...
  else if (const DeclStmt *ls = llvm::dyn_cast<DeclStmt>(stmt))
  {
    PrintStartAndEndLineNumbers(stmt, "decl");
  }
  else
  {
...
void StmtCollectionVisitor::InstruNonCompoundStmtAsStmtBody(const Stmt *stmt, std::string src_fname)
{
  ...
  {
    InstruStmtWithBraces(stmt, src_fname, 1);
  }
  else
  {
    InstruStmtWithBraces(stmt, src_fname, 0);
  }
}
...
void StmtCollectionVisitor::InstruCompoundStmtAsStmtBody(const CompoundStmt *cs, std::string src_fname, bool instru_break)
{
  ...
  for (bi = cs->body_begin(), be = cs->body_end(); bi != be; bi++)
  {
    ...
    {
      InstruStmt(*bi, src_fname, 1);
    }
    else
    {
      InstruStmt(*bi, src_fname, 0);
    }
  }
}
...
std::string get_source_text(clang::SourceRange range, const clang::SourceManager& sm) {
    ...
    //spdlog::get("Logger")->info("BeginLoc:{}", start_loc.printToString(sm));
    //spdlog::get("Logger")->info("EndLoc:{}", end_loc.printToString(sm));
...
void StmtCollectionVisitor::PrintStartAndEndLineNumbers(const Stmt *stmt, std::string type)
{
  ...
  if (stmt_begin.isValid() && stmt_end.isValid())
  {
    ...
    // SourceLocation stmt_begin_tmp = theSM.getImmediateMacroCallerLoc(stmt_begin);
    // SourceLocation stmt_end_tmp = theSM.getImmediateMacroCallerLoc(stmt_end);
    ...
    int startline = theSM.getSpellingLineNumber(stmt_begin);
    int endline = theSM.getSpellingLineNumber(stmt_end);
    if(endline==0){
      return;
    }
    std::string loc_str = std::to_string(startline);
    loc_str += ",";
    loc_str += std::to_string(endline); 

    // clang::SourceRange sourcerange1 = stmt->getSourceRange();
    // spdlog::get("Logger")->info("BeginLoc:{}", stmt_begin.printToString(theSM));
    // spdlog::get("Logger")->info("EndLoc:{}", stmt_end.printToString(theSM));
    //spdlog::get("Logger")->info("BeginLocTmp:{}", stmt_begin_tmp.printToString(theSM));
    //spdlog::get("Logger")->info("EndLocTmp:{}", stmt_end_tmp.printToString(theSM));
    //spdlog::get("Logger")->info("BeginLocisMacro:{}", stmt_begin.isMacroID());
    //spdlog::get("Logger")->info("EndLocisMacro:{}", stmt_end.isMacroID());
    //spdlog::get("Logger")->info("SourceRange:{}", get_source_text(sourcerange1, theSM));
    //spdlog::get("Logger")->info("{}", Lexer::getSourceText(CharSourceRange::getCharRange(range), theSM, theLO).str());
    //spdlog::get("Logger")->info("{}",SourceManager::GetSourceText(theSM, stmt_begin, stmt_end).str() );
