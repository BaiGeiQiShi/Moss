...
import moss.covpath.PathCoverage;
...
public class GCovBasedCodeRemover {
	...
	public static String getRemovedString(File codef, File linef, PathCoverage pcov) {
		...
		//for (Map.Entry<Integer, Integer> entry : pcov.pcMap.get(codef.getAbsolutePath().replace(".c.cov.origin.c",".c")).get("lcount").entrySet()) {
		//	lcmap.put(entry.getKey(), entry.getValue());
		//}
		String filename = codef.getAbsolutePath().replace(".c.cov.origin.c",".c");
		FilePathCoverage filePathCoverage = pcov.pcMap.get(filename);
		if (filePathCoverage.lcountCovered.length == 0) {
			for (int lineNumber : filePathCoverage.lcountAll) {
				lcmap.put(lineNumber,0);
			}
		}
		else {
			for (int lineNumber : filePathCoverage.lcountAll) {
				lcmap.put(lineNumber, filePathCoverage.IsCovered(lineNumber, true) ? 1 : 0);
			}
		}
	...
	private static Map<Integer, Integer> getStartEndLineMap(File linef, String granu) {
		...
		try {
			linef_lines = FileUtils.readLines(linef, (String) null);
		} catch (Throwable t) {
			System.err.println(t);
			t.printStackTrace();
		}
	...
	private static Map<Integer, String> getStartPropertyLineMap(File linef, String granu) {
		...
		for (String linef_line : linef_lines) {
			...
			String[] elems = null;
			if (linef_line.startsWith(granu + ":")) {
				elems = linef_line.substring((granu + ":").length()).split(","); //statement:146,148,compound -> ["146", "148", "compound"]
			} else {
				continue;
			}
	...
	/*
	 * Note that this map does not include non-code lines (led by "-") determined by
	 * gcov.
	 */
