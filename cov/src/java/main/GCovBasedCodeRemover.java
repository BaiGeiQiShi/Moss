...
public class GCovBasedCodeRemover {
	...
	public static void main(String[] args) {
		...
		boolean instru_for_removal = false;
		if (args.length > 3 && "true".equals(args[3])) {
			instru_for_removal = true;
		}
		System.out.println(getRemovedString(codef, linef, gcovf, instru_for_removal));
	}

	public static String getRemovedString(File codef, File linef, File gcovf) {
		return getRemovedString(codef, linef, gcovf, false);
	}

	public static String getRemovedString(File codef, File linef, File gcovf, boolean instru_for_removal) {
		List<String> gcov_lines = null;
		try {
			gcov_lines = FileUtils.readLines(gcovf, (String) null);
		} catch (Throwable t) {
		...
		return getRemovedString(codef, linef, lcmap, iflnmap, instru_for_removal);
	}
	...
	public static String getRemovedString(File codef, File linef, String gcovstr) {
		...
		return getRemovedString(codef, linef, GCovUtils.getLineCountMap(gcov_lines),
				GCovUtils.getInconsistentFunctionLineNameMap(gcov_lines));
	}

	// public static String getRemovedString(File codef, File linef, PathCoverage
	// pcov) {
	...
	public static String getRemovedString(File codef, File linef,
		...
		return getRemovedString(codef, linef, lcmap, iflnmap, false);
	}

	public static String getRemovedString(File codef, File linef,
			Map<Integer, Long> lcmap,
			Map<Integer, String> iflnmap,
			boolean instru_for_removal) {
		...
		Map<Integer, String> spfs_map = LineFileParser.getStartPropertyLineMap(linef, "statement"); // Starting line ->
		Map<Integer, String> epfs_map = LineFileParser.getEndPropertyLineMap(linef, "statement"); // Starting line -> stmt type

		List<String> codef_lines = null;
		try {
			codef_lines = FileUtils.readLines(codef, (String) null);
		} catch (Throwable t) {
			System.err.println(t);
			t.printStackTrace();
		}
		...
		for (int i = 1; i <= codef_lines_size; i++) {
			// System.err.print(i+" "+spfs_map.get(i)+" ");
			// System.err.println(codef_lines.get(i - 1));
			if (lcmap.get(i) != null && lcmap.get(i).longValue() == 0l) { // Non-covered
				if (ffs_map.get(i) != null) { // Is a function
					...
					if (start != -1) {
						...
						for (int k = start + 1; k < end; k++) {
							if (instru_for_removal) {
								rslt_lines.add("printf(\"Line " + removed_line_idx + " deleted.\\n\");");
								removed_line_idx += 1;
							} else {
								rslt_lines.add("");
							}
				...
				if ("compound".equals(spfs_map.get(i))) {
					// System.err.println("+Keep "+i+" for compound");
					rslt_lines.add(codef_lines.get(i-1));
					continue;
				}

				if ("decl".equals(spfs_map.get(i))) {
					// System.err.println("+Keep "+i+" for compound");
					rslt_lines.add(codef_lines.get(i-1));
					continue;
				}

				if ("switchcase".equals(spfs_map.get(i))) {
					// System.err.println("+Keep "+i+" and ; for switchcase");
				...
				if (sfs_map.get(i) != null) {
					...
					if (full_removal) {
						for (int k=i; k<=j; k++) {
							if (instru_for_removal) {
								...
								// System.err.println("-Remove "+k+" for full removal in stmt "+i);
								rslt_lines.add(""); //Remove the line
							}
						}
						i = j; //Move cursor to the end of the stmt
					}else {
						// System.err.println("+Keep "+i+" for not full removal in stmt");
					...
					// System.err.println("+Keep "+i+" for non covered and not recoreded in line file");
					rslt_lines.add(codef_lines.get(i - 1));
				}
			}
				...
				if ("label".equals(spfs_map.get(i))) {
					...
					rslt_lines.add(getSemiColonAugmentedLabelLine(codef_lines.get(i - 1)));
				} else if ("switchcase".equals(spfs_map.get(i))) {
					// System.err.print(i+" "+codef_lines.get(i - 1));
					// System.err.println(getSemiColonAugmentedLabelLine(codef_lines.get(i - 1)));
					rslt_lines.add(getSemiColonAugmentedLabelLine(codef_lines.get(i - 1)));
				} else {
